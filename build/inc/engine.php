	...
	*/
	...
	if ($page->type == 2) {
		...
		if(empty($conf->copyright_key) || $conf->copyright_key != md5($host . "_uniCopyright")):
			/*
				Убирая этот копирайт вы лишаете себя какой либо поддержки со стороны разрабочиков UNI
				...
				Пожалуйста не удаляйте его, а дайте проекту возможность развиваться дальше, и возможность того что б больше людей о нем узнало
				Неужели за всю проделанную работу для вас и "за" вас, мы не заслужили хотя бы этой ссылки? которую вы все при первой же возможности удаляете...
			*/
			$tpl->set("{unigamecms_copyright}", "<div id=\"copyright\"><br>Powered by <a title=\"Сайт разработан на движке UNI GameCMS\" href=\"https://unigamecms.ru/\" target=\"_blank\">UNI GameCMS</a> © 2018-" . date("Y") . "</div>");
		else:
